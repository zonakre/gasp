...
def meandays_by_entity(conParam, pgtable, DAY_FIELD, ENTITY_FIELD,
    ...
    for day in VALUES:
        ...
        countTbl = pandas.DataFrame(sql_query(
            conParam, QUERY), columns=[ENTITY_FIELD, COUNT_FIELD_NAME]
        )
    ...
    df_to_xls(main_table, OUTPUT_FILE)
...
def meanrowsday_by_entity(conParam, pgtable, dayField, entityField, out_file,
    ...
    data = pandas.DataFrame(sql_query(
        conParam, q), columns=entityField + [mean_field]
    )
    
    df_to_xls(data, out_file)
...
def meanday_of_periods_by_entity(conParam, pgtable, DAY_FIELD, HOUR_FIELD,
    ...
    from gasp.cpu.pnd.joins       import combine_dfs
    from gasp.fm.psql             import sql_query
    from gasp.cpu.psql.mng.fld    import get_columns_type
    from gasp.to.xls              import df_to_xls
    from gasp.cpu.psql.anls.count import count_by_period_entity
...
def meanrowsday_of_periods_by_entity(conParam, pgtable, dayField, hourField,
    ...
    data = pandas.DataFrame(sql_query(
        conParam, q), columns=entityField + periodsCols + ['nday']
    )
...
def matrix_od_mean_dist_by_group(MATRIX_OD, ORIGIN_COL, GROUP_ORIGIN_ID,
    ...
    from gasp.to.xls.pnd      import psql_to_xls
    
    db_name = get_filename(MATRIX_OD)
    create_db(conParam, db_name, overwrite=True)
    conParam["DATABASE"] = db_name
    
    TABLE = shp_to_psql_tbl(
        conParam, MATRIX_OD, epsg, pgTable="tbl_{}".format(db_name),
        api="pandas"
    )
    
    OUT_TABLE = ntbl_by_query(conParam, get_filename(RESULT_MATRIX), (
    ...
    return psql_to_xls(
        ...
        "matrix", conParam, overwrite=None
    )
