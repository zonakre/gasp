...
from gasp.pgsql.mng.fld import cols_name
...
def del_topoerror_shps(conParam, shps, epsg, outfolder):
    ...
    TABLES = shp_to_psql_tbl(conParam, shps, epsg, api="shp2pgsql")
...
def proj_clean_clip(con, dic_osm, boundary, srs, workspace):
    ...
    from gasp.to.psql import shp_to_psql_tbl
    lmt_table = shp_to_psql_tbl(con, boundary, srs)
    lmt_geom = "geom"
    
    from gasp.cpu.psql.mng.prj    import re_project
    from gasp.cpu.psql.anls.ovlay import intersection
    from gasp.cpu.psql.anls.ovlay import topologic_correction
...
def check_autofc_overlap(checkShp, epsg, conParam, outOverlaps):
    ...
    table = shp_to_psql_tbl(conParam, checkShp, epsg, api="pandas")
    ...
    ntbl_by_query(conParam, resultTable, q)
