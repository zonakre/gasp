...
def fix_geom(conParam, table, geom, out_tbl, colsSelect=None, whr=None):
    ...
    if not colsSelect:
        ...
        cols_tbl = ['{}.{}'.format(
            table, x) for x in goToList(colsSelect) if x != geom
        ]
    ...
    ntbl = ntbl_by_query(conParam, out_tbl, Q)
...
def xycols_to_geom(conP, intable, x_col, y_col, outtable,
    ...
    return ntbl_by_query(conP, outtable, (
...
def geom_to_points(conParam, table, geomCol, outTable,
    ...
    return ntbl_by_query(conParam, outTable, Q)
...
def check_endpoint_ispoint(conParam, lnhTable, pntTable, outTable,
    ...
    return ntbl_by_query(conParam, outTable, (
        "SELECT * FROM ("
...
def check_geomtype_in_table(conParam, table, geomCol='geom'):
    ...
    return int(query_to_df(conParam, (
        ...
        ") AS foo"
    ).format(geomCol, table, geomCol)).iloc[0].row_count)
...
def select_main_geom_type(conparam, table, outbl, geomCol='geom'):
    ...
    return ntbl_by_query(conparam, outbl, Q)
