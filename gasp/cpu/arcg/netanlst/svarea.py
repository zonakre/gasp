...
def service_area_polygon(net, rdv, time_interval, loc, out,
    ...
    INTERVALS = str(time_interval) if type(time_interval) == int or \
        ...
        type(time_interval) == str or type(time_interval) == unicode \
    ...
    save_servArea = copy_features(
        "servArea\\Polygons", out
    )
