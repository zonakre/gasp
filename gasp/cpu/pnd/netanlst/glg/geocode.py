...
def shp_from_address(inTbl, idAddr, addrCol, outShp,
    ...
    from gasp.fm.api.glg.geocod import get_position
    from gasp.cpu.pnd          import pnt_dfwxy_to_geodf
    from gasp.oss               import get_fileformat
    from gasp.cpu.pnd.mng.fld  import fld_types
    ...
    if tblFormat == '.xls' or tblFormat == '.xlsx':
        from gasp.fm.xls import xls_to_df
        
        tblAdr = xls_to_df(inTbl, sheet=sheet_name)
    
    elif tblFormat == '.dbf':
        from gasp.fm.dbf import dbf_to_df
        
        tblAdr = dbf_to_df(inTbl)
    
    else:
        raise ValueError('Input table has not a valid format')
    ...
    if epsg_out != 4326:
        ...
        geoAdr = project_df(geoAdr, epsg_out)
...
def address_from_featcls(inShp, outShp, epsg_in):
    ...
    from gasp.fm.api.glg.geocod    import get_address
    ...
    if epsg_in != 4326:
        geoDf = project_df(geoDf, epsg_in)
