...
def path_from_coords_to_shp(latOrigin, lngOrigin, latDest, lngDest, outshp,
    ...
    import pandas
    from gasp.fm.api.orouteserv import directions
    from gasp.to.obj            import json_obj_to_geodf
...
def servarea_from_points(pntShp, inEPSG, range, outShp,
    ...
    from gasp.fm.api.orouteserv    import isochrones
    from gasp.fm.shp               import shp_to_df
    from gasp.cpu.pnd.mng          import split_df_inN
    from gasp.cpu.pnd.mng.geomattr import pointxy_to_cols
    from gasp.mng.gen              import merge_df
    from gasp.prop.feat            import get_geom_type
    from gasp.cpu.pnd.mng.prj      import project_df
    from gasp.to.obj               import df_to_dict, dict_to_geodf
    from gasp.to.shp               import df_to_shp
    
    # SHP TO GEODATAFRAME
    pntDf = shp_to_df(pntShp)
...
def cost_od(shpOrigins, shpDestinations, epsgOrigins, epsgDestinations,
    ...
    origensDf = shp_to_df(     shpOrigins)
    destinoDf = shp_to_df(shpDestinations)
    ...
    if epsgOrigins != 4326:
        origensDf = project_df(origensDf, 4326)
    
    if epsgDestinations != 4326:
        destinoDf = project_df(destinoDf, 4326)
    ...
    return df_to_xls(RESULT, table_result)
