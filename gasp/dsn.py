...
def dsn_data_collection_by_multibuffer(inBuffers, workspace, conParam, datasource,
    ...
    from gasp.to.psql               import shp_to_psql_tbl, geodf_to_pgsql
    from gasp.to.shp                import df_to_shp
    from gasp.to.shp                import psql_to_shp
    from gasp.cpu.gdl.anls.prox.bfs import get_sub_buffers, dic_buffer_array_to_shp
    
    if datasource == 'flickr':
        from gasp.cpu.pnd.flickr import photos_location
    
    elif datasource == 'facebook':
        from gasp.cpu.pnd.fb import places_by_query
    ...
    for city in inBuffers:
        ...
        inBuffers[city]["pg_buffer"] = shp_to_psql_tbl(
...
def dsnsearch_by_cell(GRID_PNT, EPSG, RADIUS, DATA_SOURCE, conpgsql, OUTPUT_TABLE):
    ...
    from gasp.to.psql         import geodf_to_pgsql
    from gasp.to.shp          import psql_to_shp
    from gasp.cpu.psql.mng.qw import ntbl_by_query
    from gasp.to.shp          import psql_to_shp
    
    # Open GRID SHP
    GRID_DF = shp_to_df(GRID_PNT)
    GRID_DF = project_df(GRID_DF, 4326) if EPSG != 4326 else GRID_DF
    ...
    ).format(cols=", ".join(COLS), dtsrc=DATA_SOURCE))
