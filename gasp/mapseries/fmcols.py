...
def col_to_map_v1(main_mxd, lyrs_to_change, layout_elm_file, symbology_to_apply,
    ...
    * layout_elm_file = file with the indication of what text should appear
    ...
    from gasp.cpu.arcg.maps.fields import list_fields
    from gasp.cpu.arcg.maps.lyrs   import lst_layers
    from gasp.cpu.arcg.maps.graph  import get_elem_by_name
    from gasp.cpu.arcg.maps.wmap   import mxd_saveas
    
    if exportMap:
        from gasp.arcg.maps.wmap import write_map
    
    # List Cols to be maped and graphic elements to be changed
    cols_and_elements = xls_to_dict(layout_elm_file)
    
    # Prepare list of layers to modify
    lyrs_to_change = goToList(lyrs_to_change)
...
def col_to_map(main_mxd, lyrs_sourcedata, joker_field, layout_elm_file,
    ...
    * layout_elm_file = file with the indication of what text should appear
    ...
    from gasp.cpu.arcg.maps.graph import get_elem_by_name
    from gasp.cpu.arcg.maps.wmap  import mxd_saveas
    from gasp.cpu.arcg.maps.wmap  import write_map
...
def col_to_map_mlyr(main_mxd, lyrs_src, lyrs_joker, mapColsTbl, graphicsTbl,
    ...
    # Get maps to do
    mapsToDo = xls_to_dict(mapColsTbl)
    
    # Get dict with association between maps and graphics elements
    graphicElem = xls_to_dict(graphicsTbl)
...
def col_to_map_equalIntervals(main_mxd, lyrs_to_change,
    ...
    * intervals_file = file with the intervals
    ...
    from gasp.cpu.arcg.maps.fields import list_fields
    from gasp.cpu.arcg.maps.graph  import get_elem_by_name
    from gasp.cpu.arcg.maps.graph  import get_elem_by_prefix
    from gasp.cpu.arcg.maps.lyrs   import lst_layers
    from gasp.cpu.arcg.maps.wmap   import mxd_saveas
    from gasp.cpu.arcg.mng.fld     import get_equal_int
    ...
    # used?
    col_min_max = xls_to_dict(min_max_file) if min_max_file \
        else None
    
    # There is a file with the intevals to be used?
    intervals_dict = xls_to_dict(intervals_file) if \
        intervals_file else None
