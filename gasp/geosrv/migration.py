...
def pgtables_to_layer_withStyle_by_col(
    ...
    from gasp.geosrv.stores.pgsql import create_store
    from gasp.geosrv.lyrs         import publish_postgis_layer
    from gasp.geosrv.styl         import create_style
    from gasp.geosrv.styl         import list_styles
    from gasp.geosrv.styl         import del_style
    from gasp.geosrv.styl.assign  import assign_style_to_layer
    ...
    if COL_DESIGNATION or TABLE_DESIGNATION:
        from gasp.fm.xls import xls_to_dict
    
    STY_DESIGNATION = xls_to_dict(
        COL_DESIGNATION, dictValueAsList=True
    ) if COL_DESIGNATION else None
    
    LYR_DESIGNATION = xls_to_dict(
        TABLE_DESIGNATION, dictValueAsList=True
    ...
    if DATATYPE == 'QUANTITATIVE':
        if os.path.exists(sldData):
            ...
            if FF == '.xls' or FF == '.xlsx':
                from gasp.fm.xls import xls_to_dict
                colorsDict    = xls_to_dict(sldData, sheet=0)
                intervalsDict = xls_to_dict(sldData, sheet=1)
...
def pgtables_groups_to_layers(groups_of_tables, pgsql_con, workName, storeName, geoserver_con={
    ...
    # Create a new store
    storeName = pgsql_con['DATABASE'] if not storeName else storeName
    create_store(storeName, workName, pgsql_con, gs_con=geoserver_con)
